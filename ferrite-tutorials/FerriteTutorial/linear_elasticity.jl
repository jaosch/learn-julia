...
addfacetset!(grid, "top", x -> x[2] ≈ 1.0) # facet for which x[2] ≈ 1.0 for all nodes
...
function assemble_external_forces!(f_ext, dh, facetset, facetvalues, prescribed_traction)
  # Create a temporary array for the facet's local contribution to the external force
  ...
  for facet in FacetIterator(dh, facetset)
    ...
    for qp in 1:getnquadpoints(facetvalues)
      # Calculate the global coordiante of the quadrature point.
  ...
  for q_point in 1:getnquadpoints(cellvalues)
    ...
    for i in 1:getnbasefunctions(cellvalues)
      ...
      for j in 1:getnbasefunctions(cellvalues)
        ...
        ∇symNⱼ = shape_symmetric_gradient(cellvalues, q_point, j)
        ke[i, j] += (∇Nᵢ ⊡ C ⊡ ∇symNⱼ) * dΩ
      end
    end
  end
  return ke
...
function calculate_stresses(grid, dh, cv, u, C)
  ...
  for cell in CellIterator(dh)
    ...
    for q_point in 1:getnquadpoints(cv)
      ϵ = function_symmetric_gradient(cv, q_point, u, celldofs(cell))
      cell_stresses[q_point] = C ⊡ ϵ # Implicitly fills qp_stresses as well
    end
